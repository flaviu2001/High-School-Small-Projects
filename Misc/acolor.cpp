...
const int  nmax = 500005, M = 1000000007;
...
void dfs(int x)
{
    ...
    if(a+b == 0)
        ...
        add = (1ll*pd[0][b]*pd[2][a]+pd[0][a]*pd[2][b]+pd[1][a]*pd[2][b]+pd[2][a]*pd[1][b])%M;
        add = (add*(k-2))%M;
        add = (add*(k-2))%M;
        add = (add*(k-2))%M;
        add = (add*invk)%M;
...
signed main()
{
    ...
    for (int i = 1; i <= n; ++i)
        cin >> go[i][0] >> go[i][1];
    dfs(r);
    cout << ((pd[0][r]+pd[1][r]+pd[2][r]+pd[3][r])*k)%M << "\n";
    return 0;
}
